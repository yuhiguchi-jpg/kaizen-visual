...
import { ArrowRight, Check, Clock3, Image as ImageIcon, Loader2, RotateCw, Save, Sparkles } from "lucide-react";
...
const initialForm: FormState = { originalMethod: "", problem: "", beforeMinutes: "", solution: "", afterMinutes: "" };
...
export default function ImprovementCreate() {
  ...
  const isValid = Boolean(payload.originalMethod && payload.problem && payload.solution && payload.beforeMinutes > 0 && payload.afterMinutes >= 0 && form.afterMinutes !== "");
  ...
  const update = (field: keyof FormState, value: string) => { setForm(current => ({ ...current, [field]: value })); setImageUrl(undefined); };
